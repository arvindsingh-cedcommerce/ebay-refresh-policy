...
class PaymentPolicyComponent extends Component {
    ...
    feildsChangePaymentLevelTwo(key,tag = false,value){
        ...
        if( tag ) form_data[key][tag]=value;
            ...
            form_data[key] = value;
            if( key === 'immediatePay' ){
                form_data.paymentMethods = ['PayPal'];
            }
        }
    ...
    renderPaymentOption(){
        ...
        temparr.push(
            <Stack vertical={true} distribution={"fillEvenly"} key={'paymentLayout'}>
                ...
                {
                    checkbox("Immediate Pay", form_data.immediatePay, this.feildsChangePaymentLevelTwo.bind(this,'immediatePay', false))
                }
                { site_id === 'MOTORS' && errors.motors.paypal &&
                    bannerPolaris("",  <p>Paypal is a required payment method for your site</p>, "critical")
                }
                {
                    choiceList('Payment Options', form_data.immediatePay?[{label:'PayPal',value:'PayPal',disabled :true}]: options_recieved.paymentMethods,  form_data.paymentMethods,  this.feildsChangePaymentLevelTwo.bind(this,'paymentMethods', false), errors.selected?'*atleast one payment method is required':'', )
                }
                {
                    form_data.paymentMethods.indexOf('PayPal')>-1 &&
                ...
                {
                    textArea("Additional checkout instruction", form_data.description, this.feildsChangePaymentLevelTwo.bind(this,'description', false), 3, "", 'This free-form string field allows the seller to give payment instructions to the buyer. These instructions will appear on eBay\'s view item and checkout pages.')
                }
            </Stack>
        );
        return temparr;
    ...
    async getPaymentMethos(){
        ...
        if(success) {
            ...
            options_recieved.paymentMethods = [ ...prepareChoiceforArray(PaymentMethods) ];
    ...
    formValidator(){
        ...
        let Errors = 0;
            ...
            if(site_id === 'MOTORS') {
                if(form_data.paymentMethods.indexOf('PayPal')===-1){
            ...
            if(form_data.paymentMethods.length===0){
    ...
    render() {
        ...
        return (
            ...
            <AntCard  title={"Payment policy"} actions={[<AntButton type='primary' onClick={this.saveFormdata.bind(this)} loading={this.props.loader}> Save</AntButton>]}>
                        {/* <Card.Section> */}
                            {
...
export default withRouter(PaymentPolicyComponent);