...
const DashboardPolaris2 = (props) => {
  ...
  const hitDashoboardAPI = async () => {
    ...
    if (success) {
      // console.log(data.productAnalytics, connectedAccountsArray);
      ...
      connectedAccountsArray.forEach((connectedAccount, index) => {
        // console.log(Object.keys(productAnalytics).find(shopid => shopid == connectedAccount.shopId))
        if (
          ...
          // console.log(productAnalytics[connectedAccount.shopId]);
          let uploaded = 0;
          let not_uploaded = 0;
          let ended = 0;
          let error = 0;
      ...
      // console.log(tempAccounts, productAnalytics);
  ...
  const get21uniqueColors = () => {
    ...
    console.log(arr);
    console.log(`${arr} is unique : ${checkIfArrayIsUnique(arr)}`);
  ...
  // useEffect(() => {
  //   if (accountClicked) {
  //     let productDataInnerUseModified = Object.keys(productDataInnerUse).map(
  //       (shopId) => {
  //         return {
  //           shopId: shopId,
  //           uploadedProducts: productDataInnerUse[shopId]?.["uploaded"],
  //           notUploadedProducts: productDataInnerUse[shopId]?.["not_uploaded"],
  //           errorProducts: productDataInnerUse[shopId]?.["error"],
  //           endedProducts: productDataInnerUse[shopId]?.["ended"],
  //         };
  //       }
  //     );
  //     // connectedAccountsArray.map((connectedAccount) => {
  //     //   if (connectedAccount["value"] === accountClicked) {
  //     //     let matchedAccountDetails = productDataInnerUseModified.find(
  //     //       (productData) => {
  //     //         return productData["shopId"] == connectedAccount["shopId"];
  //     //       }
  //     //     );
  //     //     console.log(matchedAccountDetails);
  //     //     setAccountClickedDetails({ ...matchedAccountDetails });
  //     //   }
  //     // });
  //   }
  // }, [accountClicked]);
  return (
    <PageHeader
      ...
      ghost={true}
      extra={[
        <Button primary onClick={hitDashoboardAPI} key={"test"}>
          Refresh
        </Button>,
      ]}
          ...
          <Row gutter={[0, 8]}>
            <Col span={24}>
              ...
              >
                <div style={{ height: 300 }}>
                  ...
                  {accountClicked && accountClicked.status && (
                    <>
                      <b>{accountClicked.data.label}</b>
                      <MyResponsivePieChild accountClicked={accountClicked} />
                    </>
                  )}
                </div>
            ...
            <Col span={24}>
              <Card
                ...
                title={
                  <Title level={5}>
                    Order Counts of Accounts based on Year
                  </Title>
                }
                size="small"
                style={{ borderRadius: "8px" }}
            ...
            <Col span={24}>
              <Card
                sectioned
                title={
                  <Title level={5}>Revenue of Accounts based on Year</Title>
                }
                size="small"
                style={{ borderRadius: "8px" }}
                className="hoverCss"
...
export default withRouter(DashboardPolaris2);
