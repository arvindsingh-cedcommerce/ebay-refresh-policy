...
const FinalDashboard = (props) => {
  ...
  const getAllConnectedAccounts = async () => {
    ...
    if (accountConnectedSuccess) {
      if (Array.isArray(connectedAccountData) && connectedAccountData.length) {
        ...
        let tempArr = ebayAccounts.map((account, key) => {
          ...
          //Arvind Code--------------------------------
          ebayUsersArray.push({
            site_id: account["warehouses"][0]["site_id"],
            name: account["warehouses"][0]["name"],
            shop_id: account["id"],
          });
          //Arvind Code--------------------------------
          return accountName;
    ...
    //Arvind Code--------------------------------
    let storedEbayUsersObj = JSON.parse(localStorage.getItem("ebayUsersObj"));
    ebayUsersArray.map((users) => {
      if (storedEbayUsersObj.hasOwnProperty(users.site_id)) {
        if (storedEbayUsersObj[users.site_id] !== users.name) {
          setShopId(users.shop_id);
          setSiteId(users.site_id);
        }
      } else if (!storedEbayUsersObj.hasOwnProperty(users.site_id)) {
        setShopId(users.shop_id);
        setSiteId(users.site_id);
      }
    });
