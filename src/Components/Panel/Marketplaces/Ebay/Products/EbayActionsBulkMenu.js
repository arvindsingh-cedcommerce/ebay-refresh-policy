...
const EbayActionsBulkMenu = (props) => {
  ...
  // console.log(connectedAccountsArray)
  ...
  // match-from-eBay Array
  const [matchFromEBay,setMatchFromEBay] = useState([]);
  ...
  useEffect(()=>{
    alert()
     if(marketplace?.ebay?.shop){
      let accounts = marketplace.ebay.shop;
      for(const account in accounts) {
        if(shopId === account){
          console.log(accounts[account].data.product_settings.app_to_ebay.match_from_ebay)
          setMarketplace(accounts[account].data.product_settings.app_to_ebay.match_from_ebay)
        }
      }
    }
  },[shopId]);
