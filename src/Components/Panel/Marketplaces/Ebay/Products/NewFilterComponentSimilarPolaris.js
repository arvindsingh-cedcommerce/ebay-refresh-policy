...
import { List, Stack } from "@shopify/polaris";
import {
  Alert,
  Button,
  Collapse,
...
const NewFilterComponentSimilarPolaris = ({
  ...
  const getInitialOperatorValue = (currentFilter) => {
    ...
    if (initialMoreFiltersObj) {
      arr = Object.keys(initialMoreFiltersObj)?.filter(
        (moreFilterObj, index) => {
  ...
  useEffect(() => {
    ...
    Object.keys(filters).map((filter, index) => {
      ...
      // console.log("initialMoreFiltersObj", initialMoreFiltersObj);
      let initialFilterMainValue = getInitialOperatorValue(filter);
      if (
        initialFilterOperatorValue &&
        initialFilterMainValue &&
        initialMoreFiltersObj
      ) {
        obj[filter].operator = initialFilterOperatorValue?.operatorValue;
        obj[filter].value =
          initialMoreFiltersObj[`${initialFilterMainValue?.filter}`];
  ...
  const handleDisabled = (filterObj, currentValue, currentFilter) => {
    if (
      ["listing_id", "product_type", "brand", "tags"].includes(currentFilter)
    ) {
      if (currentValue) {
      ...
      Object.keys(filterObj).map((filter, index) => {
        if (!["price", "quantity"].includes(filter) && currentValue)
          filterObj[filter]["disabled"] = true;
        if (!currentValue) filterObj[filter]["disabled"] = false;
      });
      // if(currentValue)
      ...
      >
                ...
                <Stack vertical spacing="extraTight">
                  ...
                  {filters[filter]?.["options"] ? (
                    <Select
                      ...
                      onChange={(e) => {
                        const moreFilterObj = handleDisabled(
                          { ...filters },
                          e,
                          filter
                        );
        ...
        <Alert
          ...
          description={
            // <List type="bullet">
            //   <List.Item>
            //     eBay Item Id filter don't work along with price & quantity
            //     filter vice-versa
            //   </List.Item>
            //   <List.Item>
            //     No other filters are applicable when using sku, price, and
            //     inventory filter vice-versa
            //   </List.Item>
            // </List>
            <>
              No other filters are applicable when using sku, price, and
