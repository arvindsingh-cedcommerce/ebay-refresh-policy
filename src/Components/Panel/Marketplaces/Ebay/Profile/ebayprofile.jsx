...
import {deleteProfile, getallProfile, getFilterAttributes} from "../../../../../Apirequest/ebayApirequest/profileApi";
import {getpaginationInfo} from "../../../../../services/helperFunction";
import {getFilterforRequest} from "../Products/ebayproducthelper";

let gridApi = '';
class Ebayprofile extends Component {
    ...
    async getProfiles(){
        ...
        let { paginationProps, filtersProps, filterAttributes } = this.state;
        let { filters } = filtersProps;
        let {  pageSize : count, activePage } = paginationProps;
        let profiles = await getallProfile({ count, activePage, ...getFilterforRequest(filters), marketplace: 'ebay' });
        ...
        if(success){
            ...
            let extractRowData = extractValuesfromRequest(rows, "ebay");
    ...
    async incellElement(field, data){
        switch (field) {
            case 'edit':
                let { id } = data;
                this.redirect(`/panel/ebay/createprofile?id=${id}`);
                break;
            case 'delete':
                let {  profile_id } = data;
    ...
    render() {
        ...
        return (
            <Page fullWidth title={'Profiles'}
                  primaryAction={{content: 'Create', icon: AddMajorMonotone, onAction: this.redirect.bind(this, '/panel/ebay/createprofile')}}
            >
                <Grid
                    tag={'Profile(s)'}
                    columns={columns}
