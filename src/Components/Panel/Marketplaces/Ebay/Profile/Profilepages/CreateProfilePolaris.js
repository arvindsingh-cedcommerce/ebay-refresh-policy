...
import { getProfilebyId, saveProfile } from '../../../../../../Apirequest/ebayApirequest/profileApi'
import { getTemplates, getTemplatesPost } from '../../../../../../APIrequests/TemplatesAPI'
...
const CreateProfilePolaris = (props) => {
    ...
    // flag variable
    const [updatedConnectedAccountObject, setUpdatedConnectedAccountObject] = useState(false)
        ...
        Object.keys(connectedAccountsObject).forEach(account => {
            if (connectedAccountsObject[account]['checked']) {
                Object.keys(connectedAccountsObject[account]).forEach(field => {
                    if(!['errors','checked', 'shopId', 'siteID', 'value', 'warehouse_setting'].includes(field)) {
                        if(['category_template','shipping_policy', 'return_policy', 'payment_policy'].includes(field)) {
    ...
    const hasErrorCheck = () => {
        ...
        Object.keys(connectedAccountsObject).forEach(account => {
            if (connectedAccountsObject[account]['checked']) {
                Object.keys(connectedAccountsObject[account]['errors']).forEach(field => {
    ...
    const saveProfileAction = async () => {
        ...
        if(hasErrorCheck()) {
            ...
            Object.keys(connectedAccountsObject).forEach(account => {
                if (connectedAccountsObject[account]['checked']) {
                    let templatePolicyWarehouse = dumpTemplatePolicyWarehouse(connectedAccountsObject[account])
                    test[connectedAccountsObject[account]['shopId']] = { data: templatePolicyWarehouse, site_id: connectedAccountsObject[account]['siteID'] }
                }
            })
            let postData = {
                saveInTable: true,
    ...
    const extractShopifyWarehouses = (accounts) => {
        let temp = accounts.map(account => {
            let test = { helptext: `${account.shop_url}(${account.country})`, warehouses: [] }
            test['warehouses'] = account?.warehouses.map(warehouse => {
                return { 'label': warehouse.name, 'value': warehouse.name, 'helpText': `${account.shop_url}(${account.country})` }
            })
            return test
        })
        return temp
            ...
            Object.keys(temp).forEach(shopid => {
                if (shopid == test[account]['shopId']) {
                    ...
                    if (test[account]['checked']) {
                        tempPanes = [...tempPanes, { title: test[account]['value'], content: '', key: account, closable: false, siteID: test[account]['siteID'], status: test[account]['status']
                     }]
                    }
                }
            })
    ...
    return (
        ...
        <TextStyle variation="strong"><span
            ...
            extra={[<Button primary onClick={() => saveProfileAction()} loading={profileSaveBtnLoader}>Save</Button>]}
        >
            {/* <TabsComponent totalTabs={2} tabContents={getTabContent()} /> */}
            <AccountSelectionTab profileName={profileName} setProfileName={setProfileName} connectedAccountsObject={connectedAccountsObject} setconnectedAccountsObject={setconnectedAccountsObject} templateOptions={templateOptions} shopifyWarehouses={shopifyWarehouses} panes={panes} setPanes={setPanes} checkboxError={checkboxError} setCheckboxError={setCheckboxError} profileNameError={profileNameError} setProfileNameError={setProfileNameError} />
            <br />
            ...
            {
                Object.keys(connectedAccountsObject).some(account => connectedAccountsObject[account].checked) && 
                <NewFilterProductsTab setMinProductFlag={setMinProductFlag} setPrepareQuery={setPrepareQuery} savedQuery={prepareQuery}/>
            }
        </PageHeader>
    )
}
