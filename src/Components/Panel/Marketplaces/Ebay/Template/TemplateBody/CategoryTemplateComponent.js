...
class CategoryTemplateComponent extends Component {
    ...
    async getStorefrontcategory(){
        let { form_data } = this.state;
        let { site_id, shop_id } = form_data;
        let { success, data } = await getEbayUserDetails({ site_id: site_id, shop_id});
    ...
    async  updateAutoCompleteSelection(categoryType,key){
        ...
        this.setState(stateObj, async ()=>{
            if(getKey!=='') {
                let {success, data} = await getParentCategories({category_id :getKey, site_id : stateObj.form_data.site_id, shop_id: stateObj.form_data.shop_id});
    ...
    async getAttribute(categoryType){
        ...
        if(last_category_id){
        let { success: successattribCategorywise, data:attributeCategorywise } = await getattributesCategorywise({category_id : last_category_id, site_id : site_id, shop_id});
        ...
        if(categoryType==='primaryCategory') {
            let { success: successCategoryFeatures, data: dataCategoryFeatures } = await getcategoryFeatures({category_id :last_category_id, site_id: site_id, shop_id});
    ...
    async getCategory(categoryType){
        ...
        let { form_data } = this.state;
        let {  site_id: site_id, shop_id } = form_data;
    ...
    setCategoryOptions(data,categoryType){
        ...
        form_data[categoryType].category_mapping_options= {...category_mapping_options};
        this.setState({ form_data }, () => {
            // console.log(this.state.form_data.primaryCategory);
        });
    }
    ...
    feildsChange(tag,value){
        ...
        form_data[tag]= value;
        this.setState({ form_data }, () => { if(tag === "site_id") this.prepareCategoryandAttributes()});
    }
    ...
    CategoryfeildsChange(key,categoryType,value){
        ...
        this.setState({ form_data }, () => {
            console.log('updated', this.state.form_data.primaryCategory);
        });
        this.getCategory(categoryType);
    }
    ...
    async saveFormdata(){
        ...
        if(errorFree){
            ...
            if(returnedResponse) this.redirect('/panel/ebay/templatesUS');
        }else {
    ...
    async refreshCategory(){
        let { form_data } = this.state;
        let { site_id, level, shop_id } = form_data;
        if( site_id !== '') {
            let { success , message } = await getCategoriesApi({ site_id : site_id, level, refresh_category : true, shop_id });
    ...
    render() {
        ...
        return (
            ...
            <AntCard title={"Category template"} actions={[<AntButton type='primary' onClick={this.saveFormdata.bind(this)} loading={this.props.loader}> Save</AntButton>]}>
                {/* <Card.Section>
                    {
                ...
                <Alert showIcon type='info' description={<p><b>Category template </b>is used for assigning a category to your product along with the <b>required and optional attributes</b> which you commonly use for listing on eBay.</p>} />
                { infoBanner  &&
                // <Card.Section>
                //     {
                ...
                //     }
                // </Card.Section>
                    <Alert showIcon type='info' description={<p>Category template created below will be set as <b>default category template</b> for products but you can change your default settings from <b>Configuration</b> section of the app.</p>} />
                }
                <Divider />
                {/* <Card.Section > */}
                    <Stack vertical={false} distribution={"fillEvenly"}>
                    {
                        ...
                        {
                            select("eBay site", sites, this.feildsChange.bind(this,"site_id"), form_data.shop_id, "Please choose a site", false, false, false, "*This template will be specific to selected site only." )
                        }
                    </Stack>
                {/* </Card.Section> */}
                ...
                {form_data.site_id !== "" &&
                    ...
                    <><Divider orientation='left'>Primary Category Mapping</Divider>
                            ...
                            <Stack.Item>
                                {
                                    ...
                                    // </p>, "critical")
                                    <Alert type='error' showIcon message="Error" description={'No category is selected, Categories from root till leaf are required'} />
                                }
                                {
                                    ...
                                    // </p>, "critical")
                                    <Alert type='error' showIcon message="Error" description={'Leaf categories are missing, Categories from root till leaf are required'} />
                                }
                                {/* {
                                ...
                                <Alert message={'Category path'} description={<p>
                                        {
                                            CategoryPath('primaryCategory', form_data)
                                        }
                ...
                <><Divider orientation='left'>Attribute Mapping</Divider>
                    {(attributePrimaryspinner)? getSkeleton("categoryAttribute"):
                        <FormLayout>
                            ...
                            <AntCard title={'Required Attribute'}>
                                {/* <Card.Section> */}
                                    {
                            ...
                            //       }]}>
                            <AntCard title={'Optional Attribute'} 
                            extra={<AntButton type='primary' onClick={this.handleAdd.bind(this, 'optional', 'primaryCategory')} disabled={form_data.primaryCategory.attribute_mapping.used_ebay_attrib.length > 0 && form_data.primaryCategory.attribute_mapping.used_ebay_attrib.length >= form_data.primaryCategory.optional_attribute_count}>Add attribute</AntButton>}>
                                {/* <Card.Section> */}
                                    {
                                    ...
                                    {form_data.primaryCategory.attribute_mapping.optional_map.length === 0 &&
                                        ...
                                        <Alert type='info' showIcon description={form_data.primaryCategory.optional_attribute_count>0?"Optional attributes can be added by clicking Add attribute above":"No optional attributes found"} />
                                    }
                                {/* </Card.Section> */}
                            </AntCard>
                            // </Card>
                            ...
                            <AntCard title={'Custom Attribute'} extra={<AntButton type='primary' onClick={this.handleAdd.bind(this, 'custom', 'primaryCategory')}>Add attribute</AntButton>}>
                                <Card.Section>
                                    {
                                    ...
                                    {form_data.primaryCategory.attribute_mapping.custom_map.length === 0 &&
                                        ...
                                        <Alert type='info' description={form_data.primaryCategory.optional_attribute_count>0?"Custom attributes can be added by clicking Add attribute above":"No optional attributes found"} />
                                    }
                                </Card.Section>
                                <Card.Section>
                                    {
                ...
                <>
                    <Divider orientation='left'>Product condition</Divider>
                    {
                ...
                <>
                <Divider orientation='left'>eBay store front category</Divider>
                    {
                ...
                <>
                <Divider orientation='left'>Variation Image Settings</Divider>
                    {
                    ...
                    <Stack vertical={true} spacing={"loose"}>
                        <Stack.Item>
                            { errors.secondaryCategory.noCategorySelected &&
                                ...
                                //     No category is selected, Categories from root till leaf are required
                                // </p>, "critical")
                                <Alert message={'Error'} description={<p>
                                    No category is selected, Categories from root till leaf are required
                                    </p>} showIcon type='error' />
                            }
...
export default withRouter(CategoryTemplateComponent);